const MINT = '#00E5A0', MINT_D = '#2ECC71', GREEN = '#2ECC71', AMBER = '#FF7A45', RED = '#FF4D4D', GREY = '#8E9AAE';
...
const DAY_COLOR = { Espalda: '#4B7BE5', Pecho: '#E05C5C', Pierna: '#2ECC71', Hombro: '#9B6BD6' };
...
const SEED_PROFILE = { nombre: 'Santiago', altura: '178', peso: '74', sexo: 'Hombre' };
...
function buildDays(logs) {
  return WEEK.map(w => {
    const saved = logs[w.fecha];
    return (saved && Array.isArray(saved.ex) && saved.ex.length) ? saved : freshDay(w);
  });
}

/* ── Persistencia local ────────────────────────────────────────────────
   Todo vive en el navegador del usuario: no hay backend ni datos que
   salgan del dispositivo. Si localStorage no está disponible (modo
   privado, cuota llena) la app sigue funcionando en memoria. */
const STORE_KEY = 'pilares.v1';
...
const Store = {
  ...
  save(state) {
    try {
      ...
      WEEK.forEach((w, i) => { if (state.days[i]) logs[w.fecha] = state.days[i]; });
      ...
      window.localStorage.setItem(STORE_KEY, JSON.stringify({
        v: 1, tab: state.tab, logs,
        cuadernos: state.cuadernos, files: state.files, acts: state.acts,
        libs: state.libs, profile: state.profile,
      }));
    } catch (err) {
      /* sin almacenamiento disponible: se ignora a propósito */
    }
  },
};
...
class Component extends DCLogic {
  constructor(props) {
    ...
    const logs = (saved.logs && typeof saved.logs === 'object') ? saved.logs : {};

    this.state = {
      tab: saved.tab || (props.pantallaInicial || 'Home').toLowerCase(),
      edit: !!props.modoEdicion,
      panel: false,
      logs,
      days: buildDays(logs), activeDay: 1, expanded: null, showDone: false,
      rest: 0, restTotal: 0, restName: '', restOn: false,
      cuadernos: saved.cuadernos || SEED_CUADERNOS,
      openCuaderno: null, estudioTab: 'agenda',
      month: NOW.getMonth(), year: NOW.getFullYear(), selDay: NOW.getDate(),
      files: saved.files || SEED_FILES,
      acts: saved.acts || SEED_ACTS,
      modal: null,
      libs: saved.libs || SEED_LIBS,
      showHist: false,
      profile: saved.profile || SEED_PROFILE,
    };
  }
  ...
  componentWillUnmount() { clearInterval(this.timer); clearTimeout(this.saveTimer); }

  /** Guarda con retardo: evita escribir en disco en cada tick del cronómetro. */
  persist() {
    clearTimeout(this.saveTimer);
    this.saveTimer = setTimeout(() => Store.save(this.state), 400);
  }
  ...
  urg(tipo, fecha) {
    ...
    if (d <= 2) return { color: RED, tint: 'rgba(255,77,77,.14)', label: 'CRÍTICO' };
    if (d <= 4) return { color: AMBER, tint: 'rgba(255,122,69,.14)', label: 'PRÓXIMO' };
    if (d <= 10) return { color: MINT_D, tint: 'rgba(46,204,113,.14)', label: '1 SEMANA' };
    return { color: '#5AF2C4', tint: 'rgba(90,242,196,.12)', label: 'HOLGADO' };
  }
  plazo(fecha) {
    const d = dayDiff(TODAY, fecha);
    if (d === 0) return 'HOY';
  ...
  renderVals() {
    ...
    const exList = shown.map(e => {
      ...
      return {
        ...
        iconBg: full ? 'rgba(46,204,113,.12)' : (sec ? 'rgba(255,122,69,.08)' : 'rgba(0,229,160,.08)'),
        iconBorder: full ? 'rgba(46,204,113,.33)' : (sec ? 'rgba(255,122,69,.19)' : 'rgba(0,229,160,.19)'),
        ...
        rows: e.done.map((dn, i) => ({
          ...
          op: dn ? 0.45 : 1,
          inputBorder: dn ? 'rgba(0,229,160,.27)' : '#28324A',
    ...
    const week = WEEK.map((d, i) => {
      ...
      return {
        dow: d.dow, n: String(d.n), abbr: s.days[i].abbr,
        bg: on ? '#fff' : '#121724', border: on ? '#fff' : (i === 1 ? 'rgba(0,229,160,.33)' : '#1D2534'),
        fg: on ? '#090C14' : '#fff', sub: on ? 'rgba(10,14,26,.5)' : '#8E9AAE',
        abbrColor: on ? 'rgba(10,14,26,.55)' : (i === 1 ? ac : '#4A566B'),
        dot: self.dayPct(i) >= 1 ? GREEN : (self.dayPct(i) > 0 ? ac : 'transparent'),
        dotBorder: self.dayPct(i) > 0 ? 'transparent' : (on ? 'rgba(10,14,26,.25)' : '#28324A'),
    ...
    for (let d = 1; d <= dim; d++) {
      ...
      cells.push({
        ...
        border: sel ? 'rgba(255,255,255,.35)' : (isToday ? 'rgba(0,229,160,.5)' : 'transparent'),
    ...
    const mkLib = l => {
      ...
      return {
        ...
        bg: green ? 'rgba(46,204,113,.09)' : 'rgba(255,77,77,.08)',
        border: green ? 'rgba(0,229,160,.32)' : 'rgba(255,77,77,.3)',
        tint: green ? 'rgba(0,229,160,.14)' : 'rgba(255,77,77,.14)',
    ...
    const urgent = pendAll.slice()
      ...
      .concat(pagar.slice().sort((a, b) => (+b.monto) - (+a.monto)).slice(0, 2).map(l => ({
        badge: 'DEUDA', color: RED, tint: 'rgba(255,77,77,.14)',
    ...
    bars.forEach(b => {
      ...
      b.bg = b.i === 0 ? AMBER : (b.raw ? 'rgba(255,122,69,.28)' : '#28324A');
    ...
    return {
      ...
      exCountText: day.ex.length + ' ejercicios · ' + (pct >= 100 ? 'Sesión completada' : 'Compuestos primero'),
      week, exList,
      ...
      estudioKickerColor: s.openCuaderno ? '#FF7A45' : '#8E9AAE',
      ...
      segs: [['Agenda', 'agenda'], ['Cuadernos', 'cuadernos']].map(([t, k]) => {
        ...
        return { t, bg: on ? '#FF7A45' : 'rgba(255,255,255,.06)', border: on ? '#FF7A45' : 'rgba(255,255,255,.12)', fg: on ? '#0A0E1A' : '#8E9AAE', shadow: on ? '0 8px 18px rgba(255,122,69,.24),inset 0 1px 0 rgba(255,255,255,.3)' : 'inset 0 1px 0 rgba(255,255,255,.06)', go: () => self.setState({ estudioTab: k, openCuaderno: null }) };
      }),
      ...
      legend: [{ c: '#5AF2C4', t: '+1.5 sem' }, { c: MINT_D, t: '1 sem' }, { c: AMBER, t: '4 días' }, { c: RED, t: '≤2 días' }, { c: GREY, t: 'Tarea' }].map(x => x),
      ...
      kpis: [
        { label: 'SERIES HOY', value: doneSets + '/' + totalSets, color: AMBER, sub: day.abbr, border: 'rgba(255,122,69,.3)', go: () => self.setState({ tab: 'ejercicio' }) },
        ...
        { label: 'POR COBRAR', value: '$' + fmtMoney(sumMine), color: MINT, sub: cobrar.length + ' LIBRETICAS', border: 'rgba(0,229,160,.26)', go: () => self.setState({ tab: 'finanzas' }) },
        { label: 'POR PAGAR', value: '$' + fmtMoney(sumOwe), color: RED, sub: pagar.length + ' LIBRETICAS', border: 'rgba(255,77,77,.26)', go: () => self.setState({ tab: 'finanzas' }) },
      ],
      urgentCount: urgent.length + ' ITEMS',
      urgent,
      bars,
      debtRows: [
        { label: 'Por cobrar', sub: cobrar.length + ' libreticas · me deben', total: '$' + fmtMoney(sumMine), color: MINT, bg: 'rgba(46,204,113,.09)', border: 'rgba(0,229,160,.28)' },
        { label: 'Por pagar', sub: pagar.length + ' libreticas · yo debo', total: '$' + fmtMoney(sumOwe), color: RED, bg: 'rgba(255,77,77,.08)', border: 'rgba(255,77,77,.28)' },
      ],
      ...
      editCardBg: s.edit ? 'rgba(255,122,69,.1)' : '#121724',
      editCardBorder: s.edit ? 'rgba(255,122,69,.34)' : 'rgba(255,255,255,.07)',
      ...
      tabs: tabDefs.map(([label, key, hs]) => {
        ...
        return {
          ...
          fg: on ? '#FF7A45' : '#8E9AAE', bg: on ? 'rgba(255,122,69,.12)' : 'transparent',
          border: on ? 'rgba(255,122,69,.26)' : 'transparent',
          shadow: on ? 'inset 0 1px 0 rgba(255,122,69,.18)' : 'none',
...
Pilares.mount({
  ...
  onRender: (component) => component.persist(),
});

/* Reloj del marco de dispositivo (solo visible en escritorio). */
(function clock() {
